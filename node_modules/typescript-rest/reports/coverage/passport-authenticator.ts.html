...
<html lang="en">
<head>
    <title>Code coverage report for passport-authenticator.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="index.html">All files</a> passport-authenticator.ts
    </h1>
    <div class='clearfix'>
...
&nbsp;
    constructor(strategy: passport.Strategy, <span class="missing-if-branch" title="if path not taken" >I</span>options: PassportAuthenticatorOptions = {}) {
        this.options = options;
        const authStrategy = options.strategyName || strategy.name || <span class="branch-2 cbranch-no" title="branch not covered" >'default_strategy';</span>
...
&nbsp;
    public initialize(router: express.Router): void {
        ...
        <span class="missing-if-branch" title="else path not taken" >E</span>if (useSession) {
            router.use(passport.session());
            <span class="missing-if-branch" title="else path not taken" >E</span>if (this.options.serializeUser &amp;&amp; this.options.deserializeUser) {
                passport.serializeUser((user: any, done: (a: any, b: string) =&gt; void) =&gt; {
                    Promise.resolve(this.options.serializeUser(user))
                        .then((result: string) =&gt; {
                            done(null, result);
                        }).catch(<span class="fstat-no" title="function not covered" >(</span>err: Error) =&gt; {
<span class="cstat-no" title="statement not covered" >                            done(err, null);</span>
                        });
                });
                passport.deserializeUser(<span class="fstat-no" title="function not covered" >(</span>user: string, done: (a: any, b: any) =&gt; void) =&gt; {
<span class="cstat-no" title="statement not covered" >                    Promise.resolve(this.options.deserializeUser(user))</span>
                        .then(<span class="fstat-no" title="function not covered" >(</span>result: any) =&gt; {
